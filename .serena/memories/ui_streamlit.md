...
- With `st.form`, avoid conditional paths that leave the form without a visible `st.form_submit_button`; Streamlit will warn and interactions will not submit. Do not call `st.rerun()` from inside form rendering/submission branches if the same render can still draw stale controls. Prefer stable `on_click` callbacks that queue actions in `st.session_state`, then process the queue before rendering the affected controls.
