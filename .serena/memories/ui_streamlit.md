...
- Prompt preview should render Markdown safely and read-only; raw copy/download remains the authoritative way to transfer prompts. Keep prompt previews in a fixed-height scroll box with scoped typography so headings/code cannot overlap controls in dark mode.
