# Team B – Kahoot: Test- und Demo-Plan (User-Rolle)

**Präambel:** Die drei Test-Workflows sind von jedem Teammitglied selbstständig durchzuführen. Es geht insbesondere darum, **persönliche Arbeitsspuren in GitHub** zu hinterlassen (also alles mit eigenem Namen "Vorname@Team-B" als Label zu dokumentieren). Die Demo-Vorbereitungsphase soll dann als Team durchgeführt werden.

## Testphase – 3 Workflows (ca. 90 Minuten)

### Workflow 1: Eigenes app-spezifisches Fragenset erstellen (30 min)

1. **Prompt-Download (5 min)**
   - In der Sidebar „Fragenset erstellen“ öffnen.
   - Prompts anzeigen und einen herunterladen (⬇️ Download).
   - Inhalt prüfen, Probleme (z.B. fehlende Beispiele) dokumentieren.

2. **KI-Chatbot-Sitzung (15 min)**
   - Prompt in KI-Chatbot einfügen, 5–10 Fragen generieren.
   - Probleme notieren, JSON speichern (`custom_TeamB_set.json`).

3. **Upload und Validierung (10 min)**
   - JSON hochladen, prüfen, starten.
   - Teilen testen, Issues dokumentieren (Labels `Fragenset`, `Kahoot`).

### Workflow 2: Fragenset nach Testdurchlauf exportieren (30 min)

1. **Test durchführen (10 min)**
   - Vollständigen Test mit eigenem Set absolvieren.

2. **Export nach Testende (15 min)**
   - Zu „Kahoot!-Export“ scrollen, Button klicken, Datei speichern.
   - Probleme dokumentieren.

3. **Dokumentation (5 min)**
   - Issues für Workflow 2 erstellen.

### Workflow 3: Mit exportiertem Set eine Quiz-Runde durchführen (30 min)

1. **Import in Kahoot! (10 min)**
   - Datei importieren, Quiz prüfen (Fragen, Antworten).

2. **Quiz-Runde starten (15 min)**
   - Lobby öffnen, 5–10 Fragen spielen, Ergebnisse beobachten.
   - Probleme notieren.

3. **Dokumentation (5 min)**
   - Issues für Workflow 3 erfassen.

## Demo-Phase – Nutzerjourney präsentieren (max. 30 Minuten)

1. **Rahmen setzen (3 min)**
   - Vollständigen Workflow zeigen.

2. **Workflow 1 & 2 live (15 min)**
   - Prompt, KI-Sitzung simulieren, Upload, Test, Export.

3. **Workflow 3: Quiz-Runde (10 min)**
   - Import zeigen, kurze Runde mit 3 Fragen.

4. **Abschluss (2 min)**
   - Offene Issues markieren.

### Artefakte für die Nachbereitung

- Liste aller erstellten Issues inklusive Links.
- Exportdatei und Link zum Test-Kahoot!.
- Screenshots (Dialog, Download, Kahoot!-Lobby/Ergebnis).
